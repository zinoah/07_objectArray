...
public class MemberService {
		...
		for(int i = 0; i < memberArr.length; i++) {//회원정보가 있을 경우
			...
			if(memberArr[i] != null) {
				...
				if(memberArr[i].getMemberId().equals(memberId) && memberArr[i].getMemberPw().equals(memberPw)) {
					...
					break; // 더 이상 같은 아니디, 비밀번호 없으므로 for문 종료
				}
			}
		}//for문 끝
	...
	public void searchRegion() {
		...
		//4) 검색하는 결과가 없을경우 " 일치하는 검색 결과 없음  " 출력
